...
int main(void)
{
    ...
    */
    ...
    result = FSBank_Build(FSBANK_BUILDMODE_SINGLE, FSBANK_FORMAT_PCM, FSBANK_PLATFORM_PC, 0, 0, "car.fsb", NUMCARFILES, &carfiles[0], &carsampledefaults[0], 0, 0, 0, 1, 0, 0);
    if (result != FSBANK_OK)
    {
        printf("ERROR\n");
        return -1;
    ...
    result = FSBank_Build(FSBANK_BUILDMODE_SINGLE, FSBANK_FORMAT_PCM, FSBANK_PLATFORM_PC, 0, 0, "other.fsb", NUMOTHERFILES, &otherfiles[0], &othersampledefaults[0], 0, 0, 0, 1, 0, 0);
    printf("done.\n");
